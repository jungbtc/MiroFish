        ...
        <div class="card-content">
          ...
          <div v-if="simulationConfig && autoGeneratedRounds" class="rounds-config-section">
            <div class="rounds-header">
              ...
              <label v-if="selectedRunMode === 'full'" class="switch-control">
            ...
            <div class="run-mode-grid">
              <button
                v-for="mode in runModeOptions"
                :key="mode.id"
                type="button"
                class="run-mode-card"
                :class="{ active: selectedRunMode === mode.id }"
                @click="selectedRunMode = mode.id"
              >
                <span class="run-mode-name">{{ mode.label }}</span>
                <span class="run-mode-desc">{{ mode.description }}</span>
                <span class="run-mode-meta">{{ mode.meta }}</span>
              </button>
            </div>
            
            <Transition name="fade" mode="out-in">
              <div v-if="selectedRunMode !== 'full'" class="rounds-content auto" key="preset">
                <div class="auto-info-card">
                  <div class="auto-value">
                    <span class="val-num">{{ effectivePresetRounds }}</span>
                  ...
                  <div class="auto-content">
                    <div class="auto-meta-row">
                      <span class="duration-badge">
                        ...
                        {{ $t('step2.estimatedDuration', { minutes: Math.round(effectivePresetRounds * selectedRunModeOption.durationFactor) }) }}
                      </span>
                    </div>
                    <div class="auto-desc">
                      <p>{{ selectedRunModeOption.detail }}</p>
                    </div>
                  </div>
                </div>
              </div>

              <div v-else-if="useCustomRounds" class="rounds-content custom" key="custom">
...
const runModeOptions = computed(() => [
  {
    id: 'preview',
    label: t('step2.runModePreview'),
    description: t('step2.runModePreviewDesc'),
    detail: t('step2.runModePreviewDetail'),
    meta: t('step2.runModePreviewMeta'),
    maxRounds: 40,
    platform: 'twitter',
    durationFactor: 0.45
  },
  {
    id: 'balanced',
    label: t('step2.runModeBalanced'),
    description: t('step2.runModeBalancedDesc'),
    detail: t('step2.runModeBalancedDetail'),
    meta: t('step2.runModeBalancedMeta'),
    maxRounds: 80,
    platform: 'parallel',
    durationFactor: 0.75
  },
  {
    id: 'full',
    label: t('step2.runModeFull'),
    description: t('step2.runModeFullDesc'),
    detail: t('step2.runModeFullDetail'),
    meta: t('step2.runModeFullMeta'),
    maxRounds: null,
    platform: 'parallel',
    durationFactor: 1
  }
...
const selectedRunModeOption = computed(() => {
  return runModeOptions.value.find(mode => mode.id === selectedRunMode.value) || runModeOptions.value[0]
})

const effectivePresetRounds = computed(() => {
  const cap = selectedRunModeOption.value.maxRounds
  if (!cap) return autoGeneratedRounds.value || customMaxRounds.value
  return Math.min(autoGeneratedRounds.value || cap, cap)
})
...
const handleStartSimulation = () => {
  // 构建传递给父组件的参数
  const selectedMode = selectedRunModeOption.value
  const params = {
    runMode: selectedMode.id,
    platform: selectedMode.platform
  }
  
  if (selectedMode.id !== 'full') {
    params.maxRounds = effectivePresetRounds.value
    addLog(t('log.startSimRunMode', {
      mode: selectedMode.label,
      rounds: effectivePresetRounds.value
    }))
