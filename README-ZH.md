...
此前导入的 Token 节省控制继续属于这条核心模拟流程：

| 模拟模式 | 默认平台 | 轮次上限 | 活跃 Agent 上限 | 上下文保护 |
| --- | --- | ---: | ---: | ---: |
| Preview | Twitter | 40 | 10 | 180,000 tokens |
| Balanced | Parallel | 80 | 18 | 220,000 tokens |
| Full Fidelity | Parallel | 使用生成场景 | 无硬上限 | 240,000 tokens |

Preview 与 Balanced 还会限制每小时 Agent 活跃度；所有模式都会通过确定性的上下文保护避免重复触发超长请求。

## 🧭 可选扩展：Deep Research 决策层

MiroFish v2 是核心流程之外的**可选扩展**，不会替代本体生成、GraphRAG 构建、Agent 模拟、报告生成或深度互动。

- 在 `/` 使用核心流程：本体 → GraphRAG → Agent → 模拟 → 报告 → 互动。
- 在 `/decision` 导入已经完成且带引用的 Deep Research 报告，增加事实/假设/竞争路径、内部关键问题、决策图和备忘录视角。
- 两条流程可以共同服务同一个真实决策，任何一条都不会覆盖另一条。
- 可选决策扩展采用本地确定性处理，新增 LLM 调用与增量模型 Token 均为 **0**。
...
不再需要 `ZEP_API_KEY`。核心流程现在使用自托管 Graphiti，并通过 OpenAI 完成 LLM 与向量嵌入工作。
...
# 2. 构建当前代码并启动核心模拟栈（含 FalkorDB）
docker compose --profile graphiti up -d
```

默认会读取根目录下的 `.env`，并映射端口 `3000（前端）/5001（后端）`

如果只使用不依赖 FalkorDB 的可选确定性决策扩展，执行 `docker compose up -d` 即可。
