...
class OasisProfileGenerator:
    ...
    将Graphiti图谱中的实体转换为OASIS模拟所需的Agent Profile

    优化特性：
    1. 调用Graphiti图谱检索功能获取更丰富的上下文
    ...
    def generate_profile_from_entity(
        ...
        """
        从Graphiti实体生成OASIS Agent Profile
    ...
    def _search_zep_for_entity(self, entity: EntityNode) -> Dict[str, Any]:
        ...
        try:
            search_result = self.graph_service.search_graph(
                graph_id=self.graph_id,
                query=comprehensive_query,
                limit=30,
            )
            results["facts"] = list(dict.fromkeys(search_result.get("facts", [])))
            node_summaries = []
            for node in search_result.get("nodes", []):
                if node.get("summary"):
                    node_summaries.append(node["summary"])
                if node.get("name") and node.get("name") != entity_name:
                    node_summaries.append(f"相关实体: {node['name']}")
            results["node_summaries"] = list(dict.fromkeys(node_summaries))
    ...
    def _generate_profile_with_llm(
        ...
        for attempt in range(max_attempts):
            try:
                ...
                logger.info(
                    "LLM request start: model=%s, reasoning_effort=%s, effective_reasoning_effort=%s, component=oasis_profile, attempt=%s",
                    self.model_name,
                    self.reasoning_effort,
                    effective_reasoning_effort,
                    attempt + 1,
                ...
                logger.info(
                    "LLM request complete: model=%s, reasoning_effort=%s, effective_reasoning_effort=%s, component=oasis_profile, attempt=%s, usage=%s",
                    self.model_name,
                    self.reasoning_effort,
                    effective_reasoning_effort,
                    attempt + 1,
                ...
                logger.warning(
                    "LLM request error: model=%s, reasoning_effort=%s, effective_reasoning_effort=%s, component=oasis_profile, attempt=%s, error=%s",
                    self.model_name,
                    self.reasoning_effort,
...
- 机构账号发言要符合其身份定位"""
    ...
    def generate_profiles_from_entities(
        ...
        Args:
            ...
            graph_id: 图谱ID，用于Graphiti检索获取更丰富上下文
